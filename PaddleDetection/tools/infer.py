...
def run(FLAGS, cfg):
    ...
    if FLAGS.do_eval:
        ...
        # NOTE:
        # ImageFolder uses COCO API to parse `anno_path` when do_eval=True.
        # For VOC/XML datasets, `anno_path` is usually a label list txt, not a COCO json.
        # Give a clear error message instead of a confusing JSONDecodeError.
        if dataset.__class__.__name__ == 'ImageFolder' and hasattr(dataset, 'get_anno'):
            anno_file = dataset.get_anno()
            if anno_file and not str(anno_file).lower().endswith('.json'):
                raise ValueError(
                    "`--do_eval=true` requires COCO-style annotation json for `TestDataset` "
                    f"(got: {anno_file}). If your annotations are VOC XML, run inference without "
                    "`--do_eval` and use `tools/eval.py` for evaluation (metric: VOC).")
